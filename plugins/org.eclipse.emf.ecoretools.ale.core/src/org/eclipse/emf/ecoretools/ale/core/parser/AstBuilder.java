...
/**
 * This class parse ".ale" file and produce model intance of Implementation
 * metamodel
 */
public class AstBuilder {
	 ...
	 */
	public List<ParseResult<ModelUnit>> makeModel(List<RRootContext> rawParses, Map<RRootContext, String> sourceFiles) {
		 ...
		 */
		build.stream().map(m -> m.getRoot()).filter(m -> m != null).forEach(implemModel -> {
			List<EClass> newClasses = allNewClasses.get(implemModel.getName());
			if (newClasses != null) {
				implemModel.getClassDefinitions().stream().forEach(clsDef -> {
					Optional<EClass> cls = newClasses.stream().filter(c -> c.getName().equals(clsDef.getName()))
							.findFirst();
					if (cls.isPresent()) {
						/*
						 * cls is a copy of clsDef.fragment since it can't be contains by clsDef &
						 * qryEnv in the same time
						 */
		...
		allExtensions.stream().forEach(cls -> {
			List<EAnnotation> toResolve = cls.getEAnnotations().stream()
			...
			toResolve.stream().forEach(annot -> {
				...
				if (candidates != null) {
					Optional<ExtendedClass> searchRes = candidates.stream()
							.filter(c -> c.getBaseClass().getName().equals(finalName)).findFirst();
		...
		allAttributes.stream().filter(attr -> attr.getEAnnotation(ModelBuilder.PARSER_SOURCE) != null).forEach(attr -> {
			...
			if (oppositeType instanceof EClass) {
				...
				if (oppositeFeature == null) {
					Optional<Attribute> candidate = allExtensions.stream()
							.filter(xtd -> xtd.getBaseClass() == oppositeType)
							.flatMap(xtd -> xtd.getAttributes().stream())
							.filter(a -> a.getFeatureRef().getName().equals(opposite)).findFirst();
