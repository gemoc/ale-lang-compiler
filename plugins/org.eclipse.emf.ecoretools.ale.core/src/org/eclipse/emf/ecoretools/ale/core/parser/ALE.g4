...
rOperation : 
	rConcreteOperation | rAbstractOperation;
	
rConcreteOperation:
(rTag)* ('def' | 'override') (dispatch='dispatch')? type=rType name=Ident '(' rParameters? ')' rBlock
;

rAbstractOperation:
	(rTag)* 'def' 'abstract' type=rType name=Ident '(' rParameters? ')'
;
