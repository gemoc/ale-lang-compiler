...
public class IfImpl extends ExprImpl implements If {
  ...
  public boa.EvalRes eval(boa.Ctx ctx) {
    boa.EvalRes result;
    boa.EvalRes vcond = ((boa.EvalRes)rev.$(this.obj.getCond()).eval(ctx));
    if(vcond instanceof boa.EvalBoolRes) {
      boa.EvalBoolRes bvcond = ((boa.EvalBoolRes)vcond);
      if(bvcond.isValue()) {
        result = rev.$(this.obj.getThn()).eval(ctx);
      }
      else {
        result = rev.$(this.obj.getEls()).eval(ctx);
      }
    }
    else {
      result = null;
