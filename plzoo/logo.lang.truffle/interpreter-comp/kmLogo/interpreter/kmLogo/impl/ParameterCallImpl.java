...
public class ParameterCallImpl extends ExpressionImpl implements ParameterCall {
	...
	public double eval(Turtle turtle) {
		double result;
		result = 0.0;
		for (StackFrame frame : tmp(turtle)) {
			for (Variable var : tmp(frame)) {
	...
	@TruffleBoundary
	private Variable[] tmp(StackFrame frame) {
		return ArrayService.toArray(Variable.class, frame.getVariables());
	}

	@TruffleBoundary
	private StackFrame[] tmp(Turtle turtle) {
		return ArrayService.toArray(StackFrame.class, turtle.getCallStack().getFrames());
	}
}
